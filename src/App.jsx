...
const SCRIPT_1 = [
  ...
  { id: 7, bot: `Durante esta interacción se conversa sobre la situación actual de vivienda y se describen posibles opciones. La interacción puede mantenerse general o centrarse en circunstancias individuales.`, autoNext: 8, options: [] },
  { id: 8, bot: `Para evaluar qué opciones de apoyo podrían ser pertinentes, pueden solicitarse ciertos datos personales.`, options: [{ label: "Alex García (Nombre)", next: 9, choice: "data_name" }, { label: "301-249-1243 (Teléfono)", next: 9, choice: "data_phone" }, { label: "10.852.364 (Número de identificación)", next: 9, choice: "data_id" }, { label: "No deseo proporcionar esta información.", next: 9, choice: "data_refused" }] },
...
const SCRIPT_2 = [
  { id: 1, bot: `Hola. Entiendo muy bien que una situación relacionada con la vivienda suele sentirse muy pesada. Pasar por algo así no es fácil, y es común sentirse inseguro o no saber bien qué hacer después. Estoy aquí para ayudarle a revisar la información disponible y acompañarle, mientras exploramos juntos qué opciones podrían existir 🤍\n\n¿Preferiría empezar con un breve resumen general o revisar directamente si su situación podría encajar con el apoyo de la ONG?`, options: [{ label: "Me gustaría primero entender qué ofrecen.", next: 2, choice: "info_general" }, { label: "Prefiero verificar si esto se ajusta a mi situación.", next: 2.1, choice: "info_eligibility" }] },
  ...
  { id: 4, bot: `Gracias por compartir eso conmigo. Entiendo que pueda sentirse inseguro y que quizás no tenga claro cuál podría ser el siguiente paso.\n\nEste tipo de situación normalmente se revisa durante una evaluación inicial. En casos como este, a muchas personas les resulta útil revisar la situación con más detalle para entender qué opciones podrían existir.`, autoNext: 5, options: [] },
  { id: 5, bot: `Podemos ir revisando esto con calma, paso a paso. ¿Le gustaría conocer primero cómo suele desarrollarse una primera conversación con la ONG, o prefiere explorar qué tipos de apoyo podrían ser relevantes para su situación?`, options: [{ label: "Por favor, explique brevemente cómo funciona una conversación inicial.", next: 6, choice: "detail_process" }, { label: "Me gustaría ver de forma más concreta qué podría ser pertinente.", next: 6.1, choice: "detail_support" }] },
  ...
  { id: 7, bot: `Durante este primer contacto se conversa sobre la situación habitacional actual y se describen posibles opciones. La conversación puede mantenerse general o centrarse en circunstancias individuales.`, autoNext: 8, options: [] },
  { id: 8, bot: `Pensar en compartir datos personales en una situación así puede generar cierta incomodidad o dudas 🤍\n\nMuchas personas sienten algo parecido en este punto de la conversación.`, options: [{ label: "Alex García (Nombre)", next: 9, choice: "data_name" }, { label: "301-249-1243 (Teléfono)", next: 9, choice: "data_phone" }, { label: "10.852.364 (Número de identificación)", next: 9, choice: "data_id" }, { label: "No deseo proporcionar esta información.", next: 9, choice: "data_refused" }] },
  { id: 9, bot: `Entendido. Gracias por indicarlo.`, autoNext: 10, options: [] },
  { id: 10, bot: `Tomar una decisión en una situación así puede generar muchas dudas. Después de revisar información sobre vivienda o apoyo institucional, no siempre es fácil decidir de inmediato qué hacer, y eso es completamente comprensible.\n\n¿Le gustaría iniciar el contacto con la ONG para explorar posibles opciones de apoyo?`, options: [{ label: "Sí, me gustaría iniciar el contacto.", next: 11, choice: "1" }, { label: "No, por el momento no.", next: 11, choice: "0" }] },
  { id: 11, bot: `Muchas gracias. Ahora será dirigido a la encuesta.`, options: [] }
];

const SCRIPT_3 = [
  { id: 1, bot: `Hola. Mi función es proporcionar información inicial sobre el alcance de la ONG y explicar cómo funciona el primer contacto. Esta conversación no inicia ningún proceso formal. Después de revisar la información, usted decide si desea continuar o no.\n\n¿Desea ver primero un resumen general o revisar los criterios de elegibilidad?`, options: [{ label: "Me gustaría primero entender qué ofrecen.", next: 2, choice: "info_general" }, { label: "Prefiero verificar si esto se ajusta a mi situación.", next: 2.1, choice: "info_eligibility" }] },
...
const SCRIPT_4 = [
  { id: 1, bot: `Hola. Entiendo muy bien que una situación relacionada con la vivienda suele sentirse muy pesada. Pasar por algo así no es fácil, y es común sentirse inseguro o no saber bien qué hacer después.\n\nMi función es proporcionar información inicial sobre el alcance de la ONG y explicar cómo funciona el primer contacto. Esta conversación no inicia ningún proceso formal. Después de revisar la información, usted decide si desea continuar o no.\n\n¿Preferiría empezar con un breve resumen general o revisar directamente si su situación podría encajar con el apoyo de la ONG?`, options: [{ label: "Me gustaría primero entender qué ofrecen.", next: 2, choice: "info_general" }, { label: "Prefiero verificar si esto se ajusta a mi situación.", next: 2.1, choice: "info_eligibility" }] },
  { id: 2, bot: `Claro 😊 Empecemos con un breve panorama para que todo esto se sienta un poco más claro.`, autoNext: 3, options: [] },
  { id: 2.1, bot: `Claro 😊 Veamos primero cómo suelen evaluarse situaciones como esta, paso a paso, para que todo resulte un poco más claro.`, autoNext: 3, options: [] },
  { id: 3, bot: `La ONG apoya a estudiantes y adultos jóvenes que experimentan dificultades para mantener su situación actual de vivienda o para encontrar una vivienda asequible. El apoyo puede incluir conversaciones de orientación inicial, asesoría o remisiones a programas existentes. No hay una decisión automática en esta etapa.\n\nEl proceso comienza con una evaluación de la situación, que puede incluir la revisión de posibles opciones de alivio a corto plazo, alternativas de vivienda o servicios de orientación.`, options: [{ label: "No estoy en riesgo directo de ser desalojado, pero no estoy seguro de si podré pagar la totalidad del arriendo el próximo mes, y los meses siguientes también serán difíciles económicamente. Por eso no estoy seguro de si debería contactarlos.", next: 4, choice: "expression_uncertainty" }] },
  { id: 4, bot: `Gracias por compartir eso conmigo. Entiendo que pueda sentirse inseguro y que quizás no tenga claro cuál podría ser el siguiente paso.\n\nEste tipo de situación puede abordarse en una evaluación inicial. El propósito del primer contacto es aclarar la situación y revisar opciones posibles. Participar en esta etapa no implica ninguna obligación.\n\nEn casos como este, a muchas personas les resulta útil revisar la situación con más detalle para entender qué opciones podrían existir.`, autoNext: 5, options: [] },
  { id: 5, bot: `Podemos revisar esto con calma. ¿Le gustaría conocer primero cómo suele desarrollarse una primera conversación con la ONG, o prefiere explorar qué tipos de apoyo podrían ser pertinentes para su situación?`, options: [{ label: "Por favor, explique brevemente cómo funciona una conversación inicial.", next: 6, choice: "detail_process" }, { label: "Me gustaría ver de forma más concreta qué podría ser pertinente.", next: 6.1, choice: "detail_support" }] },
  { id: 6, bot: `Con gusto 😊 Le explico brevemente cómo suele ser ese primer intercambio para que tenga una idea más clara.`, autoNext: 7, options: [] },
  { id: 6.1, bot: `Con gusto 😊 Puedo mostrarle qué tipo de apoyo podría resultar relevante.`, autoNext: 7, options: [] },
  { id: 7, bot: `Un contacto inicial suele ser no vinculante. Durante esta interacción se conversa sobre la situación habitacional actual y se describen posibles opciones. La interacción puede mantenerse general o centrarse en circunstancias individuales.\n\nMi función aquí es informativa y orientativa. No sustituyo una consulta directa con la ONG ni tomo decisiones por usted.`, autoNext: 8, options: [] },
